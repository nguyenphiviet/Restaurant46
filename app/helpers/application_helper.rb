module ApplicationHelper
  ...
  def show_table_image table
    if table.image.present?
      image_tag table.image, width: Settings.table.sm_img_width, height: Settings.table.sm_img_height
    else
      image_tag Settings.table.default_image, width: Settings.table.sm_img_width, height: Settings.table.sm_img_height
    end
  end

  def table_image table
    if table.image.present?
      image_tag table.image, class: "img-responsive"
    else
      image_tag Settings.table.default_image, class: "img-responsive"
    end
  end
  ...
  def truncate_html html, limit
    HTML_Truncator.truncate(html, limit, :length_in_chars => true).html_safe
  end
